...
function handleDisconnect()
{
    connection = mysql.createConnection({
        host: "bssb8w95ste6bpqaxmpv-mysql.services.clever-cloud.com",
        user: "ufce6nywus7qxu74",
        password: "nSBLkz65mUZjkLxbAekR",
        database: "bssb8w95ste6bpqaxmpv",
        port: 3306
    })
    try
    {
...
app.get('/trash', isLoggedIn, (req,res)=>{
     sql = `SELECT * FROM ${md5(req.user.id)} WHERE trash = ${true}`;
    connection.query(sql, function (err, result) 
    {
        if (err) throw err;
        // console.log(result)
